...
final class Bank implements PayMethodStrategy
{
    public function __construct(
        private CustomerPaymentSubMethodRepositoryPort $customerSelectedBankRepositoryPort,
    ) {
    }
    ...
    public function createPayMethod(
        ...
        $customerSelectedBank = $this->customerSelectedBankRepositoryPort->get(
            customer: $customer,
            paymentMethodId: $paymentMethod->id
        );

        $value = $customerSelectedBank?->subPaymentMethodId
            ?? $paymentSubMethodId
            ?? '';
    ...
    {
        return $handlerIdentifier === BankHandler::class;
    }
}

...
<h1 align="center">PayU przez Store API (headless)</h1>

<p align="center">Endpointy Store API do obsługi płatności PayU w sklepach headless (Nuxt/PWA, aplikacja mobilna) — BLIK Level 0, lista banków (pay-by-link) i sprawdzanie statusu płatności.</p>

---

> 📄 Konfigurację wtyczki opisuje [główna instrukcja](index.md). Ten dokument zakłada, że wtyczka jest skonfigurowana, a metody PayU są aktywne i przypisane do kanału sprzedaży.
...
Tworzy zamówienie z bieżącego koszyka i wysyła transakcję BLIK do PayU na podstawie kodu podanego w sklepie — klient nie jest przekierowywany do bramki.

**Body:**

| Pole | Typ | Wymagane | Opis |
|---|---|---|---|
| `paymentMethodId` | string (UUID) | ✅ | ID metody płatności **BLIK** (PayU) |
...
| `redirectUrl` | string \| null | Ustawione tylko, gdy bramka wymaga dodatkowego kroku; w typowym L0 jest `null` |
...
| `providerId` | string | Identyfikator sub-metody po stronie PayU (przekazywany przy płatności) |
